...
class TestGameStatus:
    ...
    def test_get_organism_by_id(self):
        organism = Organism(position=[0, 0], radius=1)

        game_status = GameStatus(organisms=[organism])
...
class TestPosition:
    def test_valid(self):
        box = Box(x=np.array([1, 1], np.float32), y=(0, 6))

        assert isinstance(box.x, np.ndarray)
        assert isinstance(box.y, np.ndarray)

        box2 = Box(x=np.array([1, 1], np.float32), y=[0, 6])
